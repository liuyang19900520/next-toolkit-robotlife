...
export class InvestmentApi {
  // 获取投资列表
  static async getList(params?: Partial<Investment>): Promise<ApiResponse<Investment[]>> {
    try {
      // 构建查询参数
  ...
  static async getById(id: number): Promise<ApiResponse<Investment>> {
    try {
      const response = await axios.get<ApiResponse<Investment>>(`${BASE_URL}/${id}`);
      return response.data;
    } catch (error) {
  ...
  // 创建新投资
  static async create(investment: Omit<Investment, 'id'>): Promise<ApiResponse<Investment>> {
    try {
      // 确保 URL 正确，不重复路径
      const url = BASE_URL; // BASE_URL 已经是 "/api/investment"
      console.log('InvestmentApi.create - Request URL:', url);
      const response = await axios.post<ApiResponse<Investment>>(url, investment);
      return response.data;
    } catch (error) {
  ...
  static async update(
    ...
    try {
      ...
      const response = await axios.put<ApiResponse<Investment>>(url, updateData);
      return response.data;
    } catch (error) {
  ...
  static async delete(id: number): Promise<ApiResponse<void>> {
    try {
      const response = await axios.delete<ApiResponse<void>>(`${BASE_URL}/${id}`);
      return response.data;
    } catch (error) {
