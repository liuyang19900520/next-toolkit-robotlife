...
import { documentClient, tableName } from './dynamoClient';
import type { InvestmentPayload } from './validators';
...
const buildFilterExpressions = (filters: Record<string, unknown> = {}): FilterExpressions => {
  ...
  for (const [key, value] of Object.entries(filters)) {
    ...
    if (key === 'year' && typeof value === 'string') {
      // 允许按照年份前缀查询（如输入 "2025" 可以查出 "202512"）
...
export const investmentRepository = {
  ...
  async list({
    filters = {},
    sortBy,
  }: { filters?: Record<string, unknown>; sortBy?: string } = {}): Promise<InvestmentPayload[]> {
